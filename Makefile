...
install: $(BIND)/$(NAME)
	@echo "installing"
	@install -dZ ${DESTDIR}/etc/SDM
	@install -DZ $(BIND)/$(NAME) -t ${DESTDIR}/usr/bin
	@install -DZ $(RESD)/config.ini -t ${DESTDIR}/etc/SDM
	...
	@install -DZ $(RESD)/SDM.service -m 644 -t ${DESTDIR}/usr/lib/systemd/system
	@install -DZ $(RESD)/pam.d/SDM -m 644 -t ${DESTDIR}/etc/pam.d
...
uninstall:
	@echo "uninstalling"
	@rm -rf ${DESTDIR}/etc/SDM
	@rm -rf $(DATADIR)
	@rm -f ${DESTDIR}/usr/bin/SDM
	@rm -f ${DESTDIR}/usr/lib/systemd/system/SDM.service
	@rm -f ${DESTDIR}/etc/pam.d/SDM
